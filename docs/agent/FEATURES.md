...
| FBT (Frequently Bought Together — AI product bundle recommendations) | `fbt` | building | [apps/fbt/CONTEXT.md](./apps/fbt/CONTEXT.md) | Plan 1 of 6 (scaffold + additive schema) landed 2026-08-04; Plans 2–6 (bundle CRUD/dashboard, embeddings/sweep, admin, storefront SDK, cutover) in flight. Migrates onto an EXISTING production database with live merchants — `0001` is additive-only, double-enforced by a static source guard + `pnpm verify:fbt:additive` empirical verifier. No Shopify, no event map |
