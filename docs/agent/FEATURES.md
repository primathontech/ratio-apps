...
| FBT (Frequently Bought Together — AI bundle recommendations via OpenAI embeddings + cosine similarity) | `fbt` | building | [apps/fbt/CONTEXT.md](./apps/fbt/CONTEXT.md) | Plan 1 of 6 (foundation) complete on `feat/fbt-clean-schema`: greenfield `fbt_app` schema, scaffold, install/bootstrap, inbound webhooks. Migrating from standalone repo `osapp-freq-bought`; greenfield replaces an earlier in-place design (ADR 0006). Plans 2–6 (API/config, recos engine, admin, SDK, cutover) outstanding — not installable end-to-end yet |
