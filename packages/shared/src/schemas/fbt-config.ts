 ...
 *
 * Deliberately an enum rather than a cron expression: merchants set this from the
 * admin, and accepting arbitrary cron strings would mean shipping a parser, a
 * validator, and the ability for a merchant to set `* * * * *` and exhaust the
 ...
 *
 * Write-side shape only. `nextRunAt` / `lastRunAt` are absent by design — they are
 * server-owned scheduling state and must never be client-supplied.
 */
