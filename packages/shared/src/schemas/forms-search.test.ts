...
import {
  formsAutocompleteResultSchema,
  formsSearchResultSchema,
  formsStorefrontConfigSchema,
} from './forms-search';

describe('forms-search schemas', () => {
  it('parses an autocomplete payload', () => {
    const r = formsAutocompleteResultSchema.parse({
      payload: {
  ...
  it('parses a search payload with facets', () => {
    const r = formsSearchResultSchema.parse({
      payload: {
        result: [],
        total: 0,
        pages: 0,
  ...
  it('parses storefront config and rejects a stray secret', () => {
    const c = formsStorefrontConfigSchema.parse({
      storeId: 's1',
      apiKey: 'pub',
      version: '0.1.0',
    ...
    expect(() =>
      formsStorefrontConfigSchema.parse({
        storeId: 's1',
        apiKey: 'pub',
        version: '0.1.0',
        inputSelector: '#search',
