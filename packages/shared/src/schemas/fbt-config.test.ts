...
describe('fbtMerchantConfigSchema', () => {
  ...
  it('requires syncWeekday when syncFrequency is weekly', () => {
    const weeklyNoDay = fbtMerchantConfigSchema.safeParse({
      syncFrequency: 'weekly',
      syncWeekday: null,
    });
    expect(weeklyNoDay.success).toBe(false);

    const weeklyWithDay = fbtMerchantConfigSchema.safeParse({
      syncFrequency: 'weekly',
      syncWeekday: 3,
    });
    expect(weeklyWithDay.success).toBe(true);
  });
