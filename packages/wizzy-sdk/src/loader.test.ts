...
describe('loader', () => {
  ...
  function setup() {
    document.head.innerHTML =
      '<script id="wizzy-sdk" src="https://cdn.example.com/wizzy/sdk/wizzy-loader.js?store=m1"></script>';
  ...
  it('fetches the config from the script origin and stashes it on window', async () => {
    ...
    await bootWizzy();
    expect(fetchMock).toHaveBeenCalledWith('https://cdn.example.com/wizzy/sdk/config/m1');
    // @ts-expect-error test global
    expect(window.__WIZZY__.storeId).toBe('s1');
  });
  ...
  it('does nothing when searchEnabled is false', async () => {
    document.head.innerHTML =
      '<script src="https://cdn.example.com/wizzy/sdk/wizzy-loader.js?store=m1"></script>';
