import type { WizzyFacet, WizzyProduct } from '@ratio-app/shared';
import { css, html, LitElement, unsafeCSS } from 'lit';
import { customElement, property, state } from 'lit/decorators.js';
import type { CommonFilter, WizzyClient } from '../client';
import './facet-list';
import './facet-range';
import './product-card';
...
/**
 * `<wizzy-results-page>` — the full search results page.
 *
 ...
 * Facet changes (via the bubbling `wizzy-facet-change` event) are assembled
 * into a Wizzy CommonFilter model and re-applied through `client.filter`.
 *
 * Ships as its own ESM bundle (`dist/wizzy-results.js`) injected by the loader
 * on the results route — it is NOT part of the overlay widget graph.
 */
@customElement('wizzy-results-page')
export class WizzyResultsPage extends LitElement {
  ...
  @state() private facets: WizzyFacet[] = [];
  ...
  override connectedCallback(): void {
    ...
    this.addEventListener('wizzy-facet-change', this.onFacetChange as EventListener);
  }

  override disconnectedCallback(): void {
    this.removeEventListener('wizzy-facet-change', this.onFacetChange as EventListener);
  ...
  private renderFacet(f: WizzyFacet) {
    if (f.type === 'range') {
      return html`<wizzy-facet-range .facetKey=${f.key} .label=${f.label}></wizzy-facet-range>`;
    }
...
declare global {
  interface HTMLElementTagNameMap {
    'wizzy-results-page': WizzyResultsPage;
  }
}
