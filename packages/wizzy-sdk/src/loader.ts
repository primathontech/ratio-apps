...
declare global {
  interface Window {
    __WIZZY__?: WizzyStorefrontConfig;
  }
}
...
function findScript(): HTMLScriptElement | null {
  return (
    ...
    document.querySelector<HTMLScriptElement>('script[src*="wizzy-loader.js"]')
  );
}
...
/**
 * Boot the Wizzy storefront SDK: read config from the script origin, stash it
 * on `window.__WIZZY__`, and (when search is enabled) lazily inject the widget
 * module on first input focus, with an idle/timeout fallback.
 */
export async function bootWizzy(): Promise<void> {
  ...
  const res = await fetch(`${origin}/wizzy/sdk/config/${store}`);
  if (!res.ok) return;
  const cfg = (await res.json()) as WizzyStorefrontConfig;

  window.__WIZZY__ = cfg;
  ...
  function injectWidget(): void {
    if (injected) return;
    injected = true;
    injectScript(`${origin}/wizzy/sdk/wizzy-widget.js?v=${cfg.version}`, 'data-wizzy-widget');
  }
  ...
  if (onResultsPage) {
    injectScript(`${origin}/wizzy/sdk/wizzy-results.js?v=${cfg.version}`, 'data-wizzy-results');
  }
