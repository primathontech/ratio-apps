...
const LOADER = readFileSync(distUrl('wizzy-loader.js'), 'utf8');
const WIDGET = readFileSync(distUrl('wizzy-widget.js'), 'utf8');
const RESULTS = readFileSync(distUrl('wizzy-results.js'), 'utf8');

const JS = 'text/javascript; charset=utf-8';

// ─── Mocked storefront config (matches wizzyStorefrontConfigSchema, strict) ───
const CONFIG = {
  storeId: 'm1',
  apiKey: 'pk_test',
  version: '0.1.0',
...
/** Wire all SDK-asset + Wizzy-API routes onto the page. Returns hit flags. */
async function mockRoutes(page: Page): Promise<{ filterHit: () => boolean }> {
  ...
  await page.route('**/wizzy/sdk/wizzy-loader.js*', (r) => fulfillJs(r, LOADER));
  await page.route('**/wizzy/sdk/wizzy-widget.js*', (r) => fulfillJs(r, WIDGET));
  await page.route('**/wizzy/sdk/wizzy-results.js*', (r) => fulfillJs(r, RESULTS));
  await page.route('**/wizzy/sdk/config/*', (r) =>
...
test.describe('Wizzy SDK E2E (built bundles)', () => {
  test('overlay autocomplete', async ({ page }) => {
    ...
    const overlay = page.locator('wizzy-search-overlay');
    ...
    await expect(overlay.locator('wizzy-product-card').first()).toBeVisible({ timeout: 10_000 });
  });
  ...
  test('submit navigates to results page and renders grid + facets', async ({ page }) => {
    ...
    const resultsPage = page.locator('wizzy-results-page');
    await expect(resultsPage).toBeVisible({ timeout: 10_000 });
    await expect(resultsPage.locator('wizzy-product-card').first()).toBeVisible({
      timeout: 10_000,
    });
    await expect(resultsPage.locator('wizzy-facet-list').first()).toBeVisible({ timeout: 10_000 });
  });
  ...
  test('facet filter re-queries', async ({ page }) => {
    ...
    await expect(resultsPage.locator('wizzy-product-card')).toHaveCount(3, { timeout: 10_000 });

    // Toggle the first brand checkbox inside the list facet.
    const checkbox = resultsPage
      .locator('wizzy-facet-list')
      .first()
    ...
    await expect(resultsPage.locator('wizzy-product-card')).toHaveCount(1, { timeout: 10_000 });
  });
});
