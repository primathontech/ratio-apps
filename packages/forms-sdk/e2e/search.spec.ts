...
const LOADER = readFileSync(distUrl('forms-loader.js'), 'utf8');
const WIDGET = readFileSync(distUrl('forms-widget.js'), 'utf8');
const RESULTS = readFileSync(distUrl('forms-results.js'), 'utf8');

const JS = 'text/javascript; charset=utf-8';

// ─── Mocked storefront config (matches formsStorefrontConfigSchema, strict) ───
const CONFIG = {
  storeId: 'm1',
  apiKey: 'pk_test',
  version: '0.1.0',
  inputSelector: '#search',
  resultsMountSelector: '#forms-results',
...
/** Wire all SDK-asset + Forms-API routes onto the page. Returns hit flags. */
async function mockRoutes(page: Page): Promise<{ filterHit: () => boolean }> {
  ...
  await page.route('**/forms/sdk/forms-loader.js*', (r) => fulfillJs(r, LOADER));
  await page.route('**/forms/sdk/forms-widget.js*', (r) => fulfillJs(r, WIDGET));
  await page.route('**/forms/sdk/forms-results.js*', (r) => fulfillJs(r, RESULTS));
  await page.route('**/forms/sdk/config/*', (r) =>
...
test.describe('Forms SDK E2E (built bundles)', () => {
  test('overlay autocomplete', async ({ page }) => {
    ...
    const overlay = page.locator('forms-search-overlay');
    ...
    await expect(overlay.locator('forms-product-card').first()).toBeVisible({ timeout: 10_000 });
  });
  ...
  test('submit navigates to results page and renders grid + facets', async ({ page }) => {
    ...
    const resultsPage = page.locator('forms-results-page');
    await expect(resultsPage).toBeVisible({ timeout: 10_000 });
    await expect(resultsPage.locator('forms-product-card').first()).toBeVisible({
      timeout: 10_000,
    });
    await expect(resultsPage.locator('forms-facet-list').first()).toBeVisible({ timeout: 10_000 });
  });
  ...
  test('facet filter re-queries', async ({ page }) => {
    ...
    await expect(resultsPage.locator('forms-product-card')).toHaveCount(3, { timeout: 10_000 });

    // Toggle the first brand checkbox inside the list facet.
    const checkbox = resultsPage
      .locator('forms-facet-list')
      .first()
    ...
    await expect(resultsPage.locator('forms-product-card')).toHaveCount(1, { timeout: 10_000 });
  });
});
