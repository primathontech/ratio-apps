...
import './facet-list';
import type { FormsFacetList } from './facet-list';

async function mount() {
  const el = document.createElement('forms-facet-list') as FormsFacetList;
  el.facetKey = 'brands';
  el.label = 'Brand';
...
describe('forms-facet-list', () => {
  ...
  it('dispatches forms-facet-change with the new selection on toggle', async () => {
    const el = await mount();
    const onChange = vi.fn();
    el.addEventListener('forms-facet-change', (e) => onChange((e as CustomEvent).detail));
