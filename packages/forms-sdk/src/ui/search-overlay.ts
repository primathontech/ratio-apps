// TEMPLATE: Lit `<forms-search-overlay>` component. Customize the custom-element tag and the suggestion/product shapes to match this vendor.
import type { FormsProduct, FormsSuggestion } from '@ratio-app/shared';
import { css, html, LitElement } from 'lit';
import { customElement, property, state } from 'lit/decorators.js';
import type { FormsClient } from '../client';
import type { RecentStore } from '../recent-store';
import './product-card';
import { baseStyles, themeVars } from './theme';

/**
 * `<forms-search-overlay>` — the search dropdown rendered beneath a storefront's
 * search input.
 *
 * Two display states:
 ...
 *
 * Picking a chip/category or submitting fires a composed `forms-submit`
 * `CustomEvent<{ q: string }>`; the widget entry handles navigation.
 */
@customElement('forms-search-overlay')
export class FormsSearchOverlay extends LitElement {
  ...
  @state() private query = '';
  @state() private categories: FormsSuggestion[] = [];
  @state() private suggestions: FormsSuggestion[] = [];
  ...
  /** Fire a composed `forms-submit` event with the trimmed query and close. */
  submit(q: string): void {
    const t = q.trim();
    if (!t) return;
    this.dispatchEvent(
      new CustomEvent('forms-submit', { detail: { q: t }, bubbles: true, composed: true }),
    );
    this.open = false;
  }
      ...
      <div class="wz-section">
        ...
        <div class="wz-grid">
          ${this.products.map(
            (p) => html`
              <div class="wz-prod">
                <forms-product-card .product=${p}></forms-product-card>
...
declare global {
  interface HTMLElementTagNameMap {
    'forms-search-overlay': FormsSearchOverlay;
  }
}
