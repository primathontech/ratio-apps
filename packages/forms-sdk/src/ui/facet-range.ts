// TEMPLATE: Lit `<forms-facet-range>` component. Customize the custom-element tag and the facet shape to match this vendor.
import { css, html, LitElement } from 'lit';
import { customElement, property, state } from 'lit/decorators.js';
import { baseStyles } from './theme';

/**
 * `<forms-facet-range>` — a numeric min/max range facet.
 *
 * Renders a label and two number inputs. On either input changing, both values
 * are re-read (empty → undefined) and a `forms-facet-change` event is dispatched
 * with `{ key, range }`, where `range` includes only the defined bounds.
 *
 * Standalone: used only by the lazy-loaded results page.
 */
@customElement('forms-facet-range')
export class FormsFacetRange extends LitElement {
...
declare global {
  interface HTMLElementTagNameMap {
    'forms-facet-range': FormsFacetRange;
  }
}
