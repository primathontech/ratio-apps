...
import './facet-range';
import type { FormsFacetRange } from './facet-range';

async function mount() {
  const el = document.createElement('forms-facet-range') as FormsFacetRange;
...
describe('forms-facet-range', () => {
  ...
  it('dispatches forms-facet-change with a range on change', async () => {
    const el = await mount();
