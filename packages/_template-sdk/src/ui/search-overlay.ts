// TEMPLATE: Lit `<__slug__-search-overlay>` component. Customize the custom-element tag and the suggestion/product shapes to match this vendor.
import type { __Slug__Product, __Slug__Suggestion } from '@ratio-app/shared';
import { css, html, LitElement } from 'lit';
import { customElement, property, state } from 'lit/decorators.js';
import type { __Slug__Client } from '../client';
import type { RecentStore } from '../recent-store';
import './product-card';
import { baseStyles, themeVars } from './theme';

/**
 * `<__slug__-search-overlay>` — the search dropdown rendered beneath a storefront's
 * search input.
 *
 * Two display states:
 ...
 *
 * Picking a chip/category or submitting fires a composed `__slug__-submit`
 * `CustomEvent<{ q: string }>`; the widget entry handles navigation.
 */
@customElement('__slug__-search-overlay')
export class __Slug__SearchOverlay extends LitElement {
  ...
  @state() private query = '';
  @state() private categories: __Slug__Suggestion[] = [];
  @state() private suggestions: __Slug__Suggestion[] = [];
  ...
  /** Fire a composed `__slug__-submit` event with the trimmed query and close. */
  submit(q: string): void {
    const t = q.trim();
    if (!t) return;
    this.dispatchEvent(
      new CustomEvent('__slug__-submit', { detail: { q: t }, bubbles: true, composed: true }),
    );
    this.open = false;
  }
      ...
      <div class="wz-section">
        ...
        <div class="wz-grid">
          ${this.products.map(
            (p) => html`
              <div class="wz-prod">
                <__slug__-product-card .product=${p}></__slug__-product-card>
...
declare global {
  interface HTMLElementTagNameMap {
    '__slug__-search-overlay': __Slug__SearchOverlay;
  }
}
