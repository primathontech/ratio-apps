...
import './results-page';
import type { __Slug__ResultsPage } from './results-page';
...
async function mount() {
  const el = document.createElement('__slug__-results-page') as __Slug__ResultsPage;
...
describe('__slug__-results-page', () => {
  it('runs the search and renders a product grid + total', async () => {
    ...
    expect(el.shadowRoot!.querySelectorAll('__slug__-product-card')).toHaveLength(1);
  ...
  it('renders a facet-list for the brands facet (values from filterSuggestions) and a facet-range for price', async () => {
    const el = await mount();
    const list = el.shadowRoot!.querySelector('__slug__-facet-list');
    const range = el.shadowRoot!.querySelector('__slug__-facet-range');
