...
import type { __Slug__SearchOverlay } from './search-overlay';
...
async function mount(recent: string[]) {
  const el = document.createElement('__slug__-search-overlay') as __Slug__SearchOverlay;
...
describe('__slug__-search-overlay', () => {
  beforeEach(() => {
  ...
  it('submit dispatches a __slug__-submit event with the query', async () => {
    const el = await mount([]);
    const onSubmit = vi.fn();
    el.addEventListener('__slug__-submit', (e) => onSubmit((e as CustomEvent).detail));
    el.submit('creatine');
