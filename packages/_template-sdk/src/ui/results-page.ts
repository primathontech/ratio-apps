// TEMPLATE: Lit `<__slug__-results-page>` component. Customize the custom-element tag and the search/facet result shapes to match this vendor.
import type { __Slug__Facet, __Slug__Product } from '@ratio-app/shared';
import { css, html, LitElement, unsafeCSS } from 'lit';
import { customElement, property, state } from 'lit/decorators.js';
import type { CommonFilter, __Slug__Client } from '../client';
import './facet-list';
import './facet-range';
import './product-card';
...
/**
 * `<__slug__-results-page>` — the full search results page.
 *
 ...
 * Facet changes (via the bubbling `__slug__-facet-change` event) are assembled
 * into a __Slug__ CommonFilter model and re-applied through `client.filter`.
 *
 * Ships as its own ESM bundle (`dist/__slug__-results.js`) injected by the loader
 * on the results route — it is NOT part of the overlay widget graph.
 */
@customElement('__slug__-results-page')
export class __Slug__ResultsPage extends LitElement {
  ...
  @property({ attribute: false }) client!: __Slug__Client;
  @property() query = '';
  @property() themePrimary = '#0fb3a9';

  @state() private products: __Slug__Product[] = [];
  @state() private total = 0;
  @state() private pages = 0;
  @state() private facets: __Slug__Facet[] = [];
  ...
  override connectedCallback(): void {
    ...
    this.addEventListener('__slug__-facet-change', this.onFacetChange as EventListener);
  }

  override disconnectedCallback(): void {
    this.removeEventListener('__slug__-facet-change', this.onFacetChange as EventListener);
  ...
  private renderFacet(f: __Slug__Facet) {
    if (f.type === 'range') {
      return html`<__slug__-facet-range .facetKey=${f.key} .label=${f.label}></__slug__-facet-range>`;
    }
    return html`<__slug__-facet-list
      .facetKey=${f.key}
      .label=${f.label}
...
declare global {
  interface HTMLElementTagNameMap {
    '__slug__-results-page': __Slug__ResultsPage;
  }
}
