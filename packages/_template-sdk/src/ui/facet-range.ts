// TEMPLATE: Lit `<__slug__-facet-range>` component. Customize the custom-element tag and the facet shape to match this vendor.
import { css, html, LitElement } from 'lit';
import { customElement, property, state } from 'lit/decorators.js';
import { baseStyles } from './theme';

/**
 * `<__slug__-facet-range>` — a numeric min/max range facet.
 *
 * Renders a label and two number inputs. On either input changing, both values
 * are re-read (empty → undefined) and a `__slug__-facet-change` event is dispatched
 * with `{ key, range }`, where `range` includes only the defined bounds.
 *
 * Standalone: used only by the lazy-loaded results page.
 */
@customElement('__slug__-facet-range')
export class __Slug__FacetRange extends LitElement {
  ...
  private emit() {
    ...
    this.dispatchEvent(
      new CustomEvent('__slug__-facet-change', {
...
declare global {
  interface HTMLElementTagNameMap {
    '__slug__-facet-range': __Slug__FacetRange;
  }
}
