...
export default defineConfig({
  ...
  server: {
    port: 5173,
    proxy: {
      '/delhivery/api': { target: 'http://localhost:3000', changeOrigin: true },
      '/delhivery/sdk': { target: 'http://localhost:3000', changeOrigin: true },
      '/delhivery/auth': { target: 'http://localhost:3000', changeOrigin: true },
    },
  },
  test: {
