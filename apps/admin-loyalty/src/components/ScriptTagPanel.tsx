...
export function ScriptTagPanel({ merchantId }: Props) {
  ...
  const apiBase = rawBase.endsWith('/') ? `${rawBase.slice(0, -1)}/loyalty` : `${rawBase}/loyalty`;
  const scriptUrl = `${apiBase}/sdk/${merchantId}.js`;
  const scriptTag = `<Script src="${scriptUrl}" strategy="afterInteractive" />`;
  const pixelConfigLine = `"loyalty-ratio": {},`;
  ...
  return (
    ...
    >
      <Typography.Paragraph strong style={{ marginBottom: 4 }}>
        1. Add the script to <Typography.Text code>src/app/layout.tsx</Typography.Text> (with the
        other pixel SDKs):
      ...
      <Typography.Paragraph type="secondary" style={{ marginBottom: 0 }}>
        The PixelRuntime then activates this SDK on the next page load; config is served from this
        app.
      </Typography.Paragraph>
    </Card>
  );
