...
function EventsPage() {
  ...
  if (!data) return <Typography.Text>Configure Forms credentials first.</Typography.Text>;
          ...
          <Space direction="vertical" size="middle" style={{ display: 'flex' }}>
            {!credentialsReady && (
              <Alert
                type="warning"
                showIcon
                message="Add your Forms API key on the Config page before saving the event mapping."
              />
            )}
            <EventMapTable />
            {update.error && (
