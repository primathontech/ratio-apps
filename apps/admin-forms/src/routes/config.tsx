...
import { DEFAULT_FORMS_HOSTS } from '@shared/constants/forms-events';
import { formsConfigInputSchema } from '@shared/schemas/forms-config';
...
//
// We resolve against `formsConfigInputSchema` (not the strict schema)
...
type ConfigInput = z.input<typeof formsConfigInputSchema>;
type ConfigOutput = z.output<typeof formsConfigInputSchema>;
...
function ConfigPage() {
  ...
  const form = useForm<ConfigInput, unknown, ConfigOutput>({
    resolver: zodResolver(formsConfigInputSchema),
    defaultValues: {
      apiKey: '',
      host: DEFAULT_FORMS_HOSTS[0],
      debug: false,
  ...
  return (
    <FormProvider {...form}>
      <Card
        title="Forms credentials"
        extra={
          <Typography.Text type="secondary">
            Forms → Project Settings → Project API Key (starts with <code>phc_</code>)
          </Typography.Text>
        }
      >
        <form
          onSubmit={form.handleSubmit(
            ...
            (errors) => {
              ...
              console.warn('Forms config form validation failed:', errors);
              form.setError('root', {
                type: 'validation',
                message:
        ...
        >
          <Space direction="vertical" size="middle" style={{ display: 'flex' }}>
            ...
            <FieldRow
              ...
              hint="US: https://us.i.forms.com · EU: https://eu.i.forms.com · or your self-hosted https URL"
            >
              <Controller
                control={form.control}
                name="host"
                render={({ field, fieldState }) => (
                  <Input
                    {...field}
                    placeholder="https://us.i.forms.com"
                    {...(fieldState.invalid ? { status: 'error' as const } : {})}
                  />
                )}
              />
