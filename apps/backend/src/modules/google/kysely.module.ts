...
export const GOOGLE_DB_TOKEN = Symbol.for('ratio-app:google:db');

/**
 * Per-module Kysely client. Each module owns its own MySQL pool against its
 * own database (`RATIO_GOOGLE_DATABASE_URL`). Registers a `'google'` probe
 ...
 * `GoogleModule` (and that module imports this one in its `imports[]`).
...
@Module({
  providers: [
    {
      provide: GOOGLE_DB_TOKEN,
      inject: [ConfigService],
      useFactory: (config: ConfigService<Env, true>): KyselyClient<GoogleDatabase> => {
        const url = config.get('RATIO_GOOGLE_DATABASE_URL' as never, {
          infer: true,
        }) as string;
        const poolSize = config.get('DB_POOL_SIZE', { infer: true }) as number;
        return createKyselyClient<GoogleDatabase>(url, { poolSize });
      },
    },
  ],
  exports: [GOOGLE_DB_TOKEN],
})
export class GoogleKyselyModule implements OnApplicationShutdown {
  constructor(
