...
import { GOOGLE_OAUTH } from '../tokens';

/**
 * Short-lived HttpOnly cookie name the admin SPA reads (via the
 * `GET /google/api/v1/oauth/install/session` endpoint below) to discover
 ...
 *
 * The `_google` suffix scopes this cookie per-module: both Google and
 * Google controllers serve from the same backend host on `path: '/'`,
 ...
 */
const INSTALL_COOKIE = 'ratio_install_merchant_google';
...
export class GoogleOAuthController {
  constructor(
    @Inject(GOOGLE_OAUTH) private readonly oauth: OAuthService<GoogleDatabase>,
    private readonly config: ConfigService<Env, true>,
  ) {}

  /**
   * Ratio redirects the merchant's browser here after they click Install on
   * the Google app card. The per-module OAuthService handles the token
  ...
  /**
   * S4: Bridges the HttpOnly `ratio_install_merchant_google` cookie to
  ...
  /**
   * Clears the `ratio_install_merchant_google` cookie. The admin SPA SHOULD
