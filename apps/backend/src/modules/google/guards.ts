...
@Injectable()
export class GoogleWebhookSignatureGuard implements CanActivate {
  private readonly inner: CanActivate;

  constructor(config: ConfigService<Env, true>) {
    const secret = config.get('RATIO_GOOGLE_CLIENT_SECRET' as never, {
      infer: true,
    }) as string;
...
@Injectable()
export class GoogleMerchantTokenGuard implements CanActivate {
  private readonly inner: CanActivate;

  constructor(
    @Inject(GOOGLE_MERCHANTS)
    merchants: MerchantsService<GoogleDatabase>,
  ) {
