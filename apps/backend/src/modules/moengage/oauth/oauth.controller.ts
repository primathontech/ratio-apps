...
import { MOENGAGE_OAUTH } from '../tokens';

/**
 * Short-lived HttpOnly cookie name the admin SPA reads (via the
 * `GET /moengage/api/v1/oauth/install/session` endpoint below) to discover
 ...
 *
 * The `_moengage` suffix scopes this cookie per-module: both PostHog and
 ...
 */
const INSTALL_COOKIE = 'ratio_install_merchant_moengage';
...
export class MoengageOAuthController {
  constructor(
    @Inject(MOENGAGE_OAUTH) private readonly oauth: OAuthService<MoengageDatabase>,
    private readonly config: ConfigService<Env, true>,
  ) {}

  /**
   * Ratio redirects the merchant's browser here after they click Install on
   * the MoEngage app card. We exchange the code for tokens, persist them
   * in the MoEngage database, then issue a short-lived HttpOnly cookie that
  ...
  async callback(
    ...
    const adminBase = this.config.get('RATIO_MOENGAGE_ADMIN_BASE_URL' as never, {
      infer: true,
    }) as string;
  ...
  /**
   * S4: Bridges the HttpOnly `ratio_install_merchant_moengage` cookie to
  ...
  /**
   * Clears the `ratio_install_merchant_moengage` cookie. The admin SPA SHOULD
