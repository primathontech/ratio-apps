...
/**
 * Per-module guard CLASSES for MoEngage.
 *
...
@Injectable()
export class MoengageWebhookSignatureGuard implements CanActivate {
  private readonly inner: CanActivate;

  constructor(config: ConfigService<Env, true>) {
    const secret = config.get('RATIO_MOENGAGE_CLIENT_SECRET' as never, {
      infer: true,
    }) as string;
...
@Injectable()
export class MoengageMerchantTokenGuard implements CanActivate {
  private readonly inner: CanActivate;

  constructor(
    @Inject(MOENGAGE_MERCHANTS)
    merchants: MerchantsService<MoengageDatabase>,
  ) {
