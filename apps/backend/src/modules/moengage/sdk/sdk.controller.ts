...
import { MoengageSdkService } from './sdk.service';

@Controller('moengage/sdk')
export class MoengageSdkController {
  constructor(private readonly sdk: MoengageSdkService) {}

  /**
   * Browser-facing endpoint. The merchant pastes
   *   <script src="https://.../moengage/sdk/<merchantId>.js" defer></script>
   ...
   * CONFIG_INVALID_DC) and 503 (PIXEL_MISSING) responses for 5 minutes,
   * poisoning CDNs during installation races. The header is set on the
   * success path inside `MoengageSdkService.render`, after every error
   * branch has already thrown.
   */
  @Get(':merchantId.js')
