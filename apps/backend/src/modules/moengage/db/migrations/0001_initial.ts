...
export async function up(db: Kysely<any>): Promise<void> {
  ...
  await db.schema
    .createTable('moengage_configs')
    .addColumn('merchant_id', 'varchar(128)', (c) => c.notNull().primaryKey())
    .addColumn('app_id', 'varchar(64)', (c) => c.notNull())
    .addColumn('data_center', 'varchar(8)', (c) => c.notNull())
    .addColumn('debug', 'boolean', (c) => c.notNull().defaultTo(false))
    .addColumn('sw_path', 'varchar(255)', (c) => c.notNull().defaultTo(''))
...
export async function down(db: Kysely<any>): Promise<void> {
  await db.schema.dropTable('moengage_configs').ifExists().execute();
