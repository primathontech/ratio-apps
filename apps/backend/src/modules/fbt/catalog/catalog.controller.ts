...
import { type FbtCatalogCollection, FbtOsStorefrontClient } from './os-storefront.client';
...
export class FbtCatalogController {
  constructor(
    ...
    private readonly collections: FbtOsStorefrontClient,
  ) {}
  ...
  async listCollections(
    ...
    @Query('page') page?: string,
    @Query('limit') limit?: string,
  ): Promise<{ items: FbtCatalogCollection[] }> {
    return {
      items: await this.collections.listCollections(merchant.id, {
        ...(search ? { search } : {}),
        page: Math.max(1, Number(page) || 1),
        limit: clampLimit(limit),
      }),
