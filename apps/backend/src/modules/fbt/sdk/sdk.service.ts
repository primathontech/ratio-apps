...
// `fbt-config` schema in packages/shared.
@Injectable()
export class FbtSdkService {
  private readonly logger = new Logger(FbtSdkService.name);
  private pixel: string | null = null;

  constructor(
    private readonly configs: FbtConfigService,
   ...
   *   2. Verify Fbt config is filled in (apiKey + host)
   *   3. Read static/fbt-pixel.js (cached after first read)
  ...
  async render(merchantId: string, reply: FastifyReply): Promise<string> {
    ...
    try {
      ...
      if (err instanceof NotFoundException) {
        throw new NotFoundException({
          message: 'merchant has not configured Fbt yet',
  ...
  private buildPrelude(merchantId: string, config: TemplateConfig): string {
    ...
    return `window.__FBT_RATIO_CONFIG__ = ${safeInlineJson(payload)};`;
  }

  private async loadPixel(): Promise<string> {
    if (this.pixel !== null) return this.pixel;
    const path = resolvePixelPath('fbt', __dirname);
    try {
