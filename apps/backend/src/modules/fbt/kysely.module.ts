...
export const FBT_DB_TOKEN = Symbol.for('ratio-app:fbt:db');

/**
 * Per-module Kysely client. Each module owns its own MySQL pool against its
 * own database (`RATIO_FBT_DATABASE_URL`). Registers a `'fbt'` probe
 ...
 * `FbtModule` (and that module imports this one in its `imports[]`).
...
@Module({
  providers: [
    {
      provide: FBT_DB_TOKEN,
      inject: [ConfigService],
      useFactory: (config: ConfigService<Env, true>): KyselyClient<FbtDatabase> => {
        const url = config.get('RATIO_FBT_DATABASE_URL' as never, {
          infer: true,
        }) as string;
        const poolSize = config.get('DB_POOL_SIZE', { infer: true }) as number;
        return createKyselyClient<FbtDatabase>(url, { poolSize });
      },
    },
  ],
  exports: [FBT_DB_TOKEN],
})
export class FbtKyselyModule implements OnApplicationShutdown {
  constructor(
