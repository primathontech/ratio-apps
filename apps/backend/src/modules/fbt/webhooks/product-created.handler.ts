...
/**
 * A new product has no embedding yet, so there is normally nothing to delete — but we
 * still clear, because a reused product id (delete → recreate) would otherwise inherit
 * the old product's vector and produce confidently-wrong recommendations.
 */
@Injectable()
