 ...
 *
 * Abstract rather than a single multi-topic handler because `WebhookHandler` exposes
 * exactly one `readonly topic` and `WebhooksService` routes on it — a multi-topic
 * handler would require changing `core/`, which AGENTS.md forbids doing for one vendor.
 *
