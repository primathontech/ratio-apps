/**
 * D3 — tighten `fbt_configs.api_key` and `fbt_configs.host` from
 * TEXT to bounded VARCHAR.
 *
 * Real Fbt project API keys are 36-50 characters; hosts top out around
 * 60 (e.g. "https://us.i.fbt.com"). VARCHAR(128) for api_key and
 ...
 * whose host exceeds 255 (Fbt hosts are URLs, capped well below 255).
 */
...
export async function up(db: Kysely<any>): Promise<void> {
  ...
  // rewrite + shared metadata lock). Safe here because fbt_configs is one
  ...
  await sql`ALTER TABLE fbt_configs
...
export async function down(db: Kysely<any>): Promise<void> {
  ...
  // safety reasoning as up(): fbt_configs is tiny.
  await sql`ALTER TABLE fbt_configs
