...
@Injectable()
export class FbtWebhookSignatureGuard implements CanActivate {
  private readonly inner: CanActivate;

  constructor(config: ConfigService<Env, true>) {
    const secret = config.get('RATIO_FBT_CLIENT_SECRET' as never, {
      infer: true,
    }) as string;
...
@Injectable()
export class FbtMerchantTokenGuard implements CanActivate {
  private readonly inner: CanActivate;

  constructor(
    @Inject(FBT_MERCHANTS)
    merchants: MerchantsService<FbtDatabase>,
  ) {
