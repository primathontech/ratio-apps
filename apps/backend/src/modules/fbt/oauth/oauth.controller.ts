...
import { FBT_OAUTH } from '../tokens';

/**
 * Short-lived HttpOnly cookie name the admin SPA reads (via the
 * `GET /fbt/api/v1/oauth/install/session` endpoint below) to discover
 ...
 *
 * The `_fbt` suffix scopes this cookie per-module: both Fbt and
 * Fbt controllers serve from the same backend host on `path: '/'`,
 ...
 */
const INSTALL_COOKIE = 'ratio_install_merchant_fbt';
...
export class FbtOAuthController {
  constructor(
    @Inject(FBT_OAUTH) private readonly oauth: OAuthService<FbtDatabase>,
    private readonly config: ConfigService<Env, true>,
  ) {}

  /**
   * Ratio redirects the merchant's browser here after they click Install on
   * the Fbt app card. The per-module OAuthService handles the token
  ...
  async callback(
    ...
    const adminBase = this.config.get('RATIO_FBT_ADMIN_BASE_URL' as never, {
      infer: true,
    }) as string;
  ...
  /**
   * S4: Bridges the HttpOnly `ratio_install_merchant_fbt` cookie to
  ...
  /**
   * Clears the `ratio_install_merchant_fbt` cookie. The admin SPA SHOULD
