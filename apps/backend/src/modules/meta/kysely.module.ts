...
export const META_DB_TOKEN = Symbol.for('ratio-app:meta:db');

/**
 * Per-module Kysely client. Each module owns its own MySQL pool against its
 * own database (`RATIO_META_DATABASE_URL`). Registers a `'meta'` probe
 ...
 * `MetaModule` (and that module imports this one in its `imports[]`).
...
@Module({
  providers: [
    {
      provide: META_DB_TOKEN,
      inject: [ConfigService],
      useFactory: (config: ConfigService<Env, true>): KyselyClient<MetaDatabase> => {
        const url = config.get('RATIO_META_DATABASE_URL' as never, {
          infer: true,
        }) as string;
        const poolSize = config.get('DB_POOL_SIZE', { infer: true }) as number;
        return createKyselyClient<MetaDatabase>(url, { poolSize });
      },
    },
  ],
  exports: [META_DB_TOKEN],
})
export class MetaKyselyModule implements OnApplicationShutdown {
  constructor(
