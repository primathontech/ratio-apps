...
/**
 * Thin SQS wrapper. Same code talks to real Amazon SQS (prod) or a local
 * SQS-compatible broker (ElasticMQ in Docker) — only env differs:
 *
 ...
 *   meta-catalog       — catalog upsert/delete ops → Meta Catalog Batch API
 *   meta-catalog-dlq   — poison/exhausted catalog ops
 *
 * Queues are ensured on boot (CreateQueue is idempotent and works on both SQS
 * and ElasticMQ; in real prod they'd also exist via IaC).
 */
...
export class QueueService implements OnModuleInit {
  ...
  async onModuleInit(): Promise<void> {
    // Best-effort: don't crash boot if the broker is down (e.g. admin-only pod).
    for (const name of Object.values(QUEUE_NAMES)) {
      try {
        await this.ensureQueue(name);
      } catch (err) {
        this.logger.warn({ msg: 'queue ensure failed (will retry on use)', name, err });
      }
    }
  }

  private async ensureQueue(name: string): Promise<string> {
