...
import { META_OAUTH } from '../tokens';

/**
 * Short-lived HttpOnly cookie name the admin SPA reads (via the
 * `GET /meta/api/v1/oauth/install/session` endpoint below) to discover
 ...
 *
 * The `_meta` suffix scopes this cookie per-module: both Meta and
 * Meta controllers serve from the same backend host on `path: '/'`,
 ...
 */
const INSTALL_COOKIE = 'ratio_install_merchant_meta';
...
export class MetaOAuthController {
  constructor(
    @Inject(META_OAUTH) private readonly oauth: OAuthService<MetaDatabase>,
    private readonly config: ConfigService<Env, true>,
  ) {}

  /**
   * Ratio redirects the merchant's browser here after they click Install on
   * the Meta app card. The per-module OAuthService handles the token
  ...
  async callback(
    ...
    const adminBase = this.config.get('RATIO_META_ADMIN_BASE_URL' as never, {
      infer: true,
    }) as string;
  ...
  /**
   * S4: Bridges the HttpOnly `ratio_install_merchant_meta` cookie to
  ...
  /**
   * Clears the `ratio_install_merchant_meta` cookie. The admin SPA SHOULD
