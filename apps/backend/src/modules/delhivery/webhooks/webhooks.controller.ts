...
import { DelhiveryWebhookSignatureGuard } from '../guards';
import { DELHIVERY_WEBHOOKS } from '../tokens';

/**
 * Inbound Ratio webhooks for Delhivery. Single endpoint per app — dispatch is
 ...
 * `RATIO_DELHIVERY_CLIENT_SECRET` via DI on first invocation. No global state.
 */
@Controller('delhivery/api/v1/oauth')
@UseGuards(DelhiveryWebhookSignatureGuard)
export class DelhiveryWebhooksController {
  constructor(
    @Inject(DELHIVERY_WEBHOOKS) private readonly webhooks: WebhooksService<DelhiveryDatabase>,
  ) {}
