 ...
 *   - delhivery_configs preserved (so reinstall restores prior settings)
 ...
 */
@Injectable()
export class DelhiveryAppUninstalledHandler implements WebhookHandler {
  // Slash-form per the platform webhook registry (docs/agent/context/
  // learnings.md — the template's dot-form `app.uninstalled` is just the
  ...
  readonly topic = DELHIVERY_WEBHOOK_TOPICS.appUninstalled;
  private readonly logger = new Logger(DelhiveryAppUninstalledHandler.name);
