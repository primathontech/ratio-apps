/**
 * D3 — tighten `delhivery_configs.api_key` and `delhivery_configs.host` from
 * TEXT to bounded VARCHAR.
 *
 * Real Delhivery project API keys are 36-50 characters; hosts top out around
 * 60 (e.g. "https://us.i.delhivery.com"). VARCHAR(128) for api_key and
 ...
 * whose host exceeds 255 (Delhivery hosts are URLs, capped well below 255).
 */
...
export async function up(db: Kysely<any>): Promise<void> {
  ...
  // rewrite + shared metadata lock). Safe here because delhivery_configs is one
...
export async function down(db: Kysely<any>): Promise<void> {
  ...
  // safety reasoning as up(): delhivery_configs is tiny.
  await sql`ALTER TABLE delhivery_configs
