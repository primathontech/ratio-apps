...
@Injectable()
export class DelhiveryWebhookSignatureGuard implements CanActivate {
  private readonly inner: CanActivate;

  constructor(config: ConfigService<Env, true>) {
    const secret = config.get('RATIO_DELHIVERY_CLIENT_SECRET' as never, {
      infer: true,
    }) as string;
...
@Injectable()
export class DelhiveryMerchantTokenGuard implements CanActivate {
  private readonly inner: CanActivate;

  constructor(
    @Inject(DELHIVERY_MERCHANTS)
    merchants: MerchantsService<DelhiveryDatabase>,
  ) {
