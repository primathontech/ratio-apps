...
import { DELHIVERY_OAUTH } from '../tokens';

/**
 * Short-lived HttpOnly cookie name the admin SPA reads (via the
 * `GET /delhivery/api/v1/oauth/install/session` endpoint below) to discover
 ...
 *
 * The `_delhivery` suffix scopes this cookie per-module: both Delhivery and
 * Delhivery controllers serve from the same backend host on `path: '/'`,
 ...
 */
const INSTALL_COOKIE = 'ratio_install_merchant_delhivery';
...
@Controller('delhivery/api/v1/oauth')
export class DelhiveryOAuthController {
  constructor(
    @Inject(DELHIVERY_OAUTH) private readonly oauth: OAuthService<DelhiveryDatabase>,
    private readonly config: ConfigService<Env, true>,
  ) {}

  /**
   * Ratio redirects the merchant's browser here after they click Install on
   * the Delhivery app card. The per-module OAuthService handles the token
  ...
  async callback(
    ...
    const adminBase = this.config.get('RATIO_DELHIVERY_ADMIN_BASE_URL' as never, {
      infer: true,
    }) as string;
  ...
  /**
   * S4: Bridges the HttpOnly `ratio_install_merchant_delhivery` cookie to
  ...
  /**
   * Clears the `ratio_install_merchant_delhivery` cookie. The admin SPA SHOULD
