...
@Injectable()
export class PosthogWebhookSignatureGuard implements CanActivate {
  private readonly inner: CanActivate;

  constructor(config: ConfigService<Env, true>) {
    const secret = config.get('RATIO_POSTHOG_CLIENT_SECRET' as never, {
      infer: true,
    }) as string;
...
@Injectable()
export class PosthogMerchantTokenGuard implements CanActivate {
  private readonly inner: CanActivate;

  constructor(
    @Inject(POSTHOG_MERCHANTS)
    merchants: MerchantsService<PosthogDatabase>,
  ) {
