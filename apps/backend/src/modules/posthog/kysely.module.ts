...
export const POSTHOG_DB_TOKEN = Symbol.for('ratio-app:posthog:db');

/**
 * Per-module Kysely client. Each module owns its own MySQL pool against its
 * own database (`RATIO_POSTHOG_DATABASE_URL`). Registers a `'posthog'` probe
 ...
 * `PosthogModule` (and that module imports this one in its `imports[]`).
...
@Module({
  providers: [
    {
      provide: POSTHOG_DB_TOKEN,
      inject: [ConfigService],
      useFactory: (config: ConfigService<Env, true>): KyselyClient<PosthogDatabase> => {
        const url = config.get('RATIO_POSTHOG_DATABASE_URL' as never, {
          infer: true,
        }) as string;
        const poolSize = config.get('DB_POOL_SIZE', { infer: true }) as number;
        return createKyselyClient<PosthogDatabase>(url, { poolSize });
      },
    },
  ],
  exports: [POSTHOG_DB_TOKEN],
})
export class PosthogKyselyModule implements OnApplicationShutdown {
  constructor(
    @Inject(POSTHOG_DB_TOKEN) private readonly handle: KyselyClient<PosthogDatabase>,
