...
export class UcDispatchController {
  ...
  @ApiOperation({
    ...
    description:
      'Direction: Unicommerce to UC connector app. Called when the seller self-ships an order. Each ' +
      'orderItemId resolves to a Ratio order via the order-item map, the shipped quantity is checked and ' +
      'decremented against what remains, the Ratio order\'s fulfillment status is set to fulfilled, or ' +
      'partially fulfilled if other items still have stock, and tracking, courier and invoice details are ' +
      'written to Ratio as metafields. Tax and GST fields are logged only, they have no Ratio destination. ' +
      'Per-item failures never abort the batch, they surface in orderItems errorMessage, with a top-level ' +
      'status of SUCCESS, PARTIAL_SUCCESS or FAILED.',
  })
  @ApiHeader({
    name: 'apikey',
    required: true,
  ...
  @ApiBody({
    required: true,
    description:
      'Items being shipped plus the self-shipping details. `orderItemId` is the id UC itself assigned at order push; ' +
      '`quantity` defaults to 1 when absent. Tax fields are "as applied" — UC sends only the ones that apply (no ' +
      "single `gstPercentage` field exists in UC's real contract).",
    schema: {
      type: 'object',
      properties: {
        orderItems: {
          ...
          items: {
            ...
            properties: {
              ...
              quantity: {
                type: 'number',
                example: 2,
                description: 'Quantity dispatched for this item. Defaults to 1 when absent.',
              },
              taxRate: {
                type: 'number',
                example: 18,
                description: 'Effective tax rate % (logged only).',
              },
              centralGstPercentage: {
                type: 'number',
                example: 9,
                description: 'CGST % (logged only).',
              },
              stateGstPercentage: {
                type: 'number',
                example: 9,
                description: 'SGST % (logged only).',
              },
              unionTerritoryGstPercentage: {
                type: 'number',
                example: 0,
                description: 'UTGST % (logged only).',
              },
              integratedGstPercentage: {
                type: 'number',
                example: 0,
                description: 'IGST % (logged only).',
              },
              compensationCessPercentage: {
                type: 'number',
                example: 0,
                description: 'Compensation cess % (logged only).',
              },
            },
        ...
        selfShipping: {
          ...
          properties: {
            deliveryPartner: {
              type: 'string',
              example: 'Delhivery',
              description: 'Logistics partner name (metafield `unicommerce.delivery_partner`).',
            },
            deliveryCourier: {
              type: 'string',
              example: 'Delhivery Surface',
              description: 'Courier service name (metafield `unicommerce.courier`).',
            },
            dispatchDate: {
              type: 'string',
              example: '2026-08-06',
              description: 'Shipment dispatch date (metafield `unicommerce.dispatch_date`).',
            },
            invoiceNumber: {
              type: 'string',
              example: 'INV-2026-0842',
              description: 'Invoice number (metafield `unicommerce.invoice_number`).',
            },
            invoiceDate: {
              type: 'string',
              example: '2026-08-06',
              description: 'Invoice date (metafield `unicommerce.invoice_date`).',
            },
            trackingId: {
              type: 'string',
              example: 'DLV-730-417-8842',
              description: 'AWB / tracking number (metafield `unicommerce.tracking_number`).',
            },
            trackingURL: {
              type: 'string',
              example: 'https://www.delhivery.com/track?awb=DLV-730-417-8842',
              description: 'Tracking URL (metafield `unicommerce.tracking_url`).',
            },
            tentativeDeliveryDate: {
              type: 'string',
              example: '2026-08-09',
              description:
                'Expected delivery date (metafield `unicommerce.tentative_delivery_date`).',
            },
          },
          required: [
            'deliveryPartner',
      ...
      required: ['orderItems', 'selfShipping'],
    },
  })
  @ApiResponse({
    status: 200,
    description:
      ...
      '`remaining quantity insufficient`, `failed to apply update`). `status` is SUCCESS when every item applied, ' +
      'PARTIAL_SUCCESS when some failed, FAILED when all failed. A disabled sync flag returns the SUCCESS shape with ' +
      'no errorMessage (accept-and-no-op).',
    schema: {
      type: 'object',
      properties: {
        ...
        orderItems: {
          ...
          items: {
            ...
            properties: {
              ...
              errorMessage: {
                type: 'string',
                example: 'remaining quantity insufficient',
