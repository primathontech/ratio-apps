...
@Injectable()
export class TemplateWebhookSignatureGuard implements CanActivate {
  private readonly inner: CanActivate;

  constructor(config: ConfigService<Env, true>) {
    const secret = config.get('RATIO_TEMPLATE_CLIENT_SECRET' as never, {
      infer: true,
    }) as string;
...
@Injectable()
export class TemplateMerchantTokenGuard implements CanActivate {
  private readonly inner: CanActivate;

  constructor(
    @Inject(TEMPLATE_MERCHANTS)
    merchants: MerchantsService<TemplateDatabase>,
  ) {
