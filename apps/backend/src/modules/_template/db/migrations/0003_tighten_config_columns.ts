/**
 * D3 — tighten `_template_configs.api_key` and `_template_configs.host` from
 * TEXT to bounded VARCHAR.
 *
 * Real Template project API keys are 36-50 characters; hosts top out around
 * 60 (e.g. "https://us.i._template.com"). VARCHAR(128) for api_key and
 ...
 * whose host exceeds 255 (Template hosts are URLs, capped well below 255).
 */
...
export async function up(db: Kysely<any>): Promise<void> {
  ...
  // rewrite + shared metadata lock). Safe here because _template_configs is one
  ...
  await sql`ALTER TABLE _template_configs
...
export async function down(db: Kysely<any>): Promise<void> {
  ...
  // safety reasoning as up(): _template_configs is tiny.
  await sql`ALTER TABLE _template_configs
