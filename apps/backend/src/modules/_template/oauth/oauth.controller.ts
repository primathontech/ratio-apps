...
import { TEMPLATE_OAUTH } from '../tokens';

/**
 * Short-lived HttpOnly cookie name the admin SPA reads (via the
 * `GET /_template/api/v1/oauth/install/session` endpoint below) to discover
 ...
 *
 * The `__template` suffix scopes this cookie per-module: both Template and
 * Template controllers serve from the same backend host on `path: '/'`,
 ...
 */
const INSTALL_COOKIE = 'ratio_install_merchant__template';
...
export class TemplateOAuthController {
  constructor(
    @Inject(TEMPLATE_OAUTH) private readonly oauth: OAuthService<TemplateDatabase>,
    private readonly config: ConfigService<Env, true>,
  ) {}

  /**
   * Ratio redirects the merchant's browser here after they click Install on
   * the Template app card. The per-module OAuthService handles the token
  ...
  async callback(
    ...
    const adminBase = this.config.get('RATIO_TEMPLATE_ADMIN_BASE_URL' as never, {
      infer: true,
    }) as string;
  ...
  /**
   * S4: Bridges the HttpOnly `ratio_install_merchant__template` cookie to
  ...
  /**
   * Clears the `ratio_install_merchant__template` cookie. The admin SPA SHOULD
