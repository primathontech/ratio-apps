...
@Injectable()
export class LoyaltyWebhookSignatureGuard implements CanActivate {
  private readonly inner: CanActivate;

  constructor(config: ConfigService<Env, true>) {
    const secret = config.get('RATIO_LOYALTY_CLIENT_SECRET' as never, {
      infer: true,
    }) as string;
...
@Injectable()
export class LoyaltyMerchantTokenGuard implements CanActivate {
  private readonly inner: CanActivate;

  constructor(
    @Inject(LOYALTY_MERCHANTS)
    merchants: MerchantsService<LoyaltyDatabase>,
  ) {
