...
import { LOYALTY_OAUTH } from '../tokens';

/**
 * Short-lived HttpOnly cookie name the admin SPA reads (via the
 * `GET /loyalty/api/v1/oauth/install/session` endpoint below) to discover
 ...
 *
 * The `_loyalty` suffix scopes this cookie per-module: both Loyalty and
 * Loyalty controllers serve from the same backend host on `path: '/'`,
 ...
 */
const INSTALL_COOKIE = 'ratio_install_merchant_loyalty';
...
@Controller('loyalty/api/v1/oauth')
export class LoyaltyOAuthController {
  constructor(
    @Inject(LOYALTY_OAUTH) private readonly oauth: OAuthService<LoyaltyDatabase>,
    private readonly config: ConfigService<Env, true>,
  ) {}

  /**
   * Ratio redirects the merchant's browser here after they click Install on
   * the Loyalty app card. The per-module OAuthService handles the token
  ...
  async callback(
    ...
    const adminBase = this.config.get('RATIO_LOYALTY_ADMIN_BASE_URL' as never, {
      infer: true,
    }) as string;
  ...
  /**
   * S4: Bridges the HttpOnly `ratio_install_merchant_loyalty` cookie to
  ...
  /**
   * Clears the `ratio_install_merchant_loyalty` cookie. The admin SPA SHOULD
