...
import { LOYALTY_CRYPTO, LOYALTY_RATIO_OAUTH_CREDS, LOYALTY_RATIO_OAUTH_HTTP } from '../tokens';
...
export class RatioTokenProvider {
  constructor(
    ...
    @Inject(LOYALTY_CRYPTO) private readonly crypto: CryptoService,
    @Inject(LOYALTY_RATIO_OAUTH_HTTP) private readonly http: RatioOAuthHttp,
    @Inject(LOYALTY_RATIO_OAUTH_CREDS) private readonly creds: RatioOAuthCreds,
  ) {}

  /**
   * `forceRefresh` bypasses the expiry check — used when Core rejects a
   * stored-but-supposedly-valid token with a 401 (clock skew, early revocation).
   */
