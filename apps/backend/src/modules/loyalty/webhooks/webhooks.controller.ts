...
import { LoyaltyWebhookSignatureGuard } from '../guards';
import { LOYALTY_WEBHOOKS } from '../tokens';

/**
 * Inbound Ratio webhooks for Loyalty. Single endpoint per app — dispatch is
 ...
 * `RATIO_LOYALTY_CLIENT_SECRET` via DI on first invocation. No global state.
 */
@Controller('loyalty/api/v1/oauth')
@UseGuards(LoyaltyWebhookSignatureGuard)
export class LoyaltyWebhooksController {
  constructor(
    @Inject(LOYALTY_WEBHOOKS) private readonly webhooks: WebhooksService<LoyaltyDatabase>,
  ) {}
