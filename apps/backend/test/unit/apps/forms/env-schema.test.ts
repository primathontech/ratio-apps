...
/**
 * DERIVED from APPS, never hand-listed. The schema requires a full
 * `RATIO_<APP>_*` block per entry, so a hardcoded fixture breaks this whole
 * suite the day someone adds an app — which is exactly how adding
 * `unicommerce` left these tests red without touching anything they test.
 */
const allAppKeys = APPS.reduce<Record<string, string>>(
  (acc, slug) => Object.assign(acc, app(slug.toUpperCase())),
  {},
);
...
describe('FORMS_* env boot validation', () => {
  ...
  it("rejects a mistyped worker flag ('True')", () => {
    expect(envSchema.safeParse({ ...validEnv, FORMS_WEBHOOK_WORKER_ENABLED: 'True' }).success).toBe(
      false,
    );
  });
