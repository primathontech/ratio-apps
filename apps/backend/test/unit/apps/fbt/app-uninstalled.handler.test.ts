...
/**
 * `trx` double covering the handler's three operations: a raw SELECT … FOR UPDATE, a
 * selectFrom read, and an updateTable write.
 *
 * The shape here is not obvious. Kysely's `sql`…`.execute(provider)` does NOT call
 * `executeQuery` on the object you hand it — `RawBuilder.execute` calls
 * `provider.getExecutor()`, then that executor's `transformQuery` → `compileQuery` →
 * `executeQuery`. A double carrying only `executeQuery` fails with
 * `executorProvider.getExecutor is not a function`.
 *
 * Wiring the real `MysqlQueryCompiler` in costs one import and makes `recordedSql` hold
 * the GENUINE compiled statement, so the test can assert the FOR UPDATE lock is actually
 * taken and that merchantId is bound as a parameter rather than interpolated.
 */
function fakeTrx(merchantRow: { id: string; isActive: boolean } | undefined) {
  ...
  const executor = {
    ...
    // biome-ignore lint/suspicious/noExplicitAny: Kysely operation-node types are internal
...
describe('FbtAppUninstalledHandler', () => {
  ...
  it('subscribes to the app/uninstalled topic', () => {
    ...
    // Pin the literal too — see topics.test.ts for why.
  ...
  // The two below cover the handler's least obvious behaviour. The FOR UPDATE
  // serialises against an in-flight OAuth callback, which also locks `merchants.id`
  // before its upsert. Without it a callback could re-INSERT `isActive = true` AFTER
  // this handler's existence check but BEFORE its UPDATE, leaving Ratio (uninstalled)
  // and the DB (active) out of sync.
