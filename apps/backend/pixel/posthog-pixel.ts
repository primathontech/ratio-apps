/*!
 * ratio-app — PostHog storefront pixel (TypeScript source)
 *
 * Compiled to `static/posthog-pixel.js` by
 ...
 *   window.__POSTHOG_RATIO_CONFIG__ = { apiKey, host, debug, merchantId, eventNameMap };
 *
 * Then the IIFE below:
 *   1. Reads that config (silently no-ops if missing)
 *   2. Loads PostHog Web SDK via the official async stub-queue snippet
 *   3. Registers with window.__OPEN_STORE_PIXEL_RUNTIME__ (or queues for it)
 *   4. Subscribes to enabled OpenStore events, maps attrs, forwards to PostHog
...
interface Window {
  __POSTHOG_RATIO_CONFIG__?: PostHogRatioConfig;
...
(() => {
  const LOG = '[PostHogRatioPixel]';

  const config = window.__POSTHOG_RATIO_CONFIG__;
  if (!config?.apiKey || !config.host) {
    console.warn(LOG, 'config missing or incomplete — pixel did not initialize.', config);
    return;
  }
  const cfg: PostHogRatioConfig = config;

  // ─── Attribute mapping (OpenStore properties → PostHog snake_case) ──────────
  ...
  // ─── PostHog Web SDK loader (official async stub-queue snippet) ─────────────
  ...
  function loadPostHog(): void {
    ...
    const e = (window.posthog ?? ([] as unknown)) as any;
    if (e.__SV) return;
    (window as Window & { posthog?: unknown }).posthog = e;
    e._i = [];
    ...
    register: (analytics) => {
      ...
      try {
        window.posthog?.init(cfg.apiKey, {
          ...
          autocapture: false,
          loaded: () => {
            if (cfg.debug) console.log(LOG, 'PostHog SDK loaded');
          },
        });
      } catch (e) {
        console.error(LOG, 'posthog.init failed:', e);
      }
        ...
        analytics.subscribe(osName, (event) => {
          try {
            ...
            if (ph && typeof ph.capture === 'function') {
              ...
              console.warn(LOG, 'PostHog handle not ready for', phName);
            }
