...
function EventsPage() {
  ...
  if (!data) return <Typography.Text>Configure PostHog credentials first.</Typography.Text>;
          ...
          <Space direction="vertical" size="middle" style={{ display: 'flex' }}>
            {!credentialsReady && (
              <Alert
                type="warning"
                showIcon
                message="Add your PostHog API key on the Config page before saving the event mapping."
              />
            )}
            <EventMapTable />
            {update.error && (
