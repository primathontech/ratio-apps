...
import { DEFAULT_POSTHOG_HOSTS } from '@shared/constants/posthog-events';
import { buildDefaultEventMap } from '@shared/schemas/event-map';
import { posthogConfigInputSchema } from '@shared/schemas/posthog-config';
...
//
// We resolve against `posthogConfigInputSchema` (not the strict schema)
...
type ConfigInput = z.input<typeof posthogConfigInputSchema>;
type ConfigOutput = z.output<typeof posthogConfigInputSchema>;
...
function ConfigPage() {
  ...
  const form = useForm<ConfigInput, unknown, ConfigOutput>({
    resolver: zodResolver(posthogConfigInputSchema),
    defaultValues: {
      apiKey: '',
      host: DEFAULT_POSTHOG_HOSTS[0],
      debug: false,
      ...
      <Card
        ...
        extra={
          <Typography.Text type="secondary">
            PostHog → Project Settings → Project API Key (starts with <code>phc_</code>)
          </Typography.Text>
        }
      >
        <form
          onSubmit={form.handleSubmit(
            ...
            (errors) => {
              ...
              console.warn('PostHog config form validation failed:', errors);
              form.setError('root', {
                type: 'validation',
                message:
        ...
        >
          <Space direction="vertical" size="middle" style={{ display: 'flex' }}>
            ...
            <FieldRow
              ...
              hint="US: https://us.i.posthog.com · EU: https://eu.i.posthog.com · or your self-hosted https URL"
            >
              <Controller
                control={form.control}
                name="host"
