...
function EventsPage() {
  ...
  if (!data) return <Typography.Text>Configure Template credentials first.</Typography.Text>;

  const credentialsReady = data.apiKey.trim().length > 0;
          ...
          <Space direction="vertical" size="middle" style={{ display: 'flex' }}>
            {!credentialsReady && (
              <Alert
                type="warning"
                showIcon
                message="Add your Template API key on the Config page before saving the event mapping."
              />
            )}
            <EventMapTable />
            {update.error && (
