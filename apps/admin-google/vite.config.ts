...
export default defineConfig({
  ...
  plugins: [
    tanstackRouter({
      ...
      // Co-located route tests (src/routes/*.test.tsx) aren't routes — keep them
      // out of the generated route tree.
  ...
  server: {
    port: 5173,
    proxy: {
      '/google/api': { target: 'http://localhost:3000', changeOrigin: true },
      '/google/sdk': { target: 'http://localhost:3000', changeOrigin: true },
      '/google/auth': { target: 'http://localhost:3000', changeOrigin: true },
    },
  },
  test: {
