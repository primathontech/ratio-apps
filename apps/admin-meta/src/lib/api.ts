...
// The unified backend mounts the Meta vendor under the `/meta/*`
// subpath. VITE_API_BASE_URL points at the backend root (e.g.
// http://localhost:3000), so we prepend `/meta` here so every call from
// this SPA lands on the correct vendor namespace.
const RAW_BASE = (import.meta.env.VITE_API_BASE_URL as string | undefined) ?? '';
const BASE = RAW_BASE.endsWith('/') ? `${RAW_BASE.slice(0, -1)}/meta` : `${RAW_BASE}/meta`;
