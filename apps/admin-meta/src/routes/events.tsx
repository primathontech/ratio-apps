...
function EventsPage() {
  ...
  if (!data) return <Typography.Text>Configure Meta credentials first.</Typography.Text>;

  const credentialsReady = data.pixelId.trim().length > 0;
          ...
          <Space direction="vertical" size="middle" style={{ display: 'flex' }}>
            {!credentialsReady && (
              <Alert
                type="warning"
                showIcon
                message="Add your Meta API key on the Config page before saving the event mapping."
              />
            )}
            <EventMapTable />
            {update.error && (
